% !TeX root = ../main.tex
%
% Body-only import derived from papers/lossy/space.tex.
...
\section{Introduction}\label{lossy:sec:intro}
...
In this chapter as well as the next, we will give a deterministic polynomial time algorithm for solving a certain class of explicit construction problems. In both chapters, the correctness of our algorithm will hinge on a certain complexity-theoretic \emph{hardness assumption}, placing these results within the rich landscape of \emph{hardness v.s. randomness} tradeoffs which have become ubiquitous in complexity theory since their introduction in \cite{Yao} and subsequent generalized treatment in \cite{NW, IW}. However, the method here (as well as in the next chapter) will exploit particular properties of the class of explicit construction problems we consider in order to achieve a derandomization under assumptions which are categorically different than those required to instantiate the general hardness v.s. randomness paradigm. The results in this chapter are based on work originally appearing in \cite{time-space}.

We motivate the main algorithm in this chapter by a certain \emph{counter-factual} thought experiment. When an explicit construction problem is defined, we have a nonconstructive proof that it is total; our starting point is to imagine a world in which some explicit construction problem is \emph{not solvable in polynomial time}, i.e. the nonconstructivity of said proof is \emph{inherent}.
...
Aside from the aforementioned conditional derandomization, another lasting contribution of the work in \cite{time-space} covered in this chapter is the introduction of the {\sc Lossy Code} problem in $\cl{TFNP}$ \todo{jerabek studied it too so maybe this is not accurate}, which has come to recent attention in connection to the study of \emph{catalytic space} \todo{explain more}. 
...
\todo{also mention the R-LOSSY thing...}


\subsection{Our Contributions}\label{lossy:subsec:contr}
...
\subsection{Relation to Prior Work}\label{lossy:subsec:prior}
...
\subsection{Known Time-Space tradeoffs:}\label{lossy:subsec:known-tradeoffs}
...
\section{Preliminaries}\label{lossy:sec:prelim}
\subsection{Basics}\label{lossy:subsec:basics}
Throughout this chapter we will define the following precise notion of an ``exponential time bound:''
\begin{definition}
...
\subsection{Machine Models}\label{lossy:subsec:machines}
...
\begin{lemma}\label{lossy:lem:ram-bst}\cite{NLT}
...
\begin{lemma}\label{lossy:lem:ram-tape-same}\cite{NLT}
...
\end{definition}
\subsection{Search Problems}\label{lossy:subsec:search-problems}
At some points in this work we will be concerned with functions that are efficiently computable with access to an oracle for solving some search problem. A subtlety arises here, since an instance of a search problem may have many valid solutions, and this may cause our oracle machine to have a variety of possible outputs. To formalize the notion of search problem oracles correctly, we need to recall the definition of a function solving a search problem from the \hyperref[chap:prelims]{preliminaries}:
\begin{definition}
...
\section{Pigeonhole Principles and Compression Schemes}\label{lossy:sec:pigeons}

\todo{reevaluate this section for redundancies during intro-unification stage}
...
\subsection{Compressors and Decompressors}\label{lossy:subsec:gi}
\begin{definition}
...
\begin{lemma}\label{lossy:lem:non-uniform-stretching}
...
\subsection{Particular Compression Schemes of Interest}\label{lossy:subsec:particular-pairs}
...
\subsubsection{Compression Schemes for Non-Uniform Complexity Measures}\label{lossy:subsubsec:tt-gens}
...
\begin{lemma}\label{lossy:lem:tt-in-apepp}
...
\subsubsection{Large Primes}\label{lossy:subsubsec:primes}
...
\begin{theorem}\label{lossy:thm:primes-in-apepp}
...
\begin{lemma}\label{lossy:lem:divisibility-coding}
...
\end{definition}
We are now ready to prove Theorem~\ref{lossy:thm:primes-in-apepp}:
\begin{proof}[Proof of Theorem~\ref{lossy:thm:primes-in-apepp}]
We will use the shorthand $N = 2^n$ for the remainder of the proof. Consider an arbitrary string $xs \in \B^{n + \lceil \log n \rceil + 4}$, where $|x| = n$ and $|s| = \lceil\log n\rceil + 4$. We will consider $x$ as an integer in the range $[N]$, and $s$ as representing a position in the interval $I_{N^{32} + x}$. Since $N^{32} + x > N^{32}$, we see that $I_{N^{32}+x}$ has length at least $32n$, and thus any $\lceil \log n \rceil + 4$ bit string can be identified uniquely with some position in $I_{N^{32}+x}$. As described above, this position is in turn fully defined by a prime power $p^k$ dividing $N^{32} + x$, together with an index $i \in [\lceil \log p \rceil]$. Further, it is clear that from $x$ and $s$ we can determine $p,k,i$ in $poly(n)$ time with a factoring oracle.
...
\end{proof}
\section{J-Trees}\label{lossy:sec:j-trees}
...
\begin{lemma}[J-tree Access Lemma]\label{lossy:lem:access}
...
\begin{lemma}[J-tree Update Lemma]\label{lossy:lem:updates}
...
\end{cases}
\]
Again, each time we evaluate $C$ on some input, we check that that input is indeed compressible with respect to $C,D$, and if not then we return $\mathsf{FAIL}\langle e\rangle$ where $e$ is the incompressible string we found. If we get to the end of this process without returning failure, we return the string $v_1$. This completes the description of the $\mathsf{Find}$ procedure, which overall requires at most $O(nk)$ time to compute using at most $O(k)$ evaluations of $C$ and $D$. Figures \ref{lossy:fig:write-f} and \ref{lossy:fig:write-b} illustrate this procedure for a J-tree of depth 3.
...
\begin{lemma}\label{lossy:lem:initialization}(J-Tree Initialization Lemma)
...
\begin{lemma}[J-Tree Iterated Compression Lemma]\label{lossy:lem:compress}
...
\subsection{Proofs of Main Theorems}\label{lossy:subsec:main-proofs}

\begin{theorem}\label{lossy:thm:low-space-sim}
...
\begin{theorem}\label{lossy:thm:swap-d-for-np}
...
\end{theorem}
\begin{proof}
We follow the same simulation $\mathcal{S}_{\epsilon}$ of some language $L$ in $\cl{RAM\dash TIME}[T(n)]$, with a slight modification. The key observation is the following: the procedure $\mathsf{Find}$ is proven to work with access to both $\mathcal{C}$ and $\mathcal{D}$ oracles by Lemma~\ref{lossy:lem:updates}. However, recall that Lemma~\ref{lossy:lem:updates} also defines separate procedure $\mathsf{Verify}$, which is able to verify that a certain J-tree seed is the updated form of another, and this verification only needs the $\mathcal{D}$ oracle. Recall also the key property relating $\mathsf{Find}$ and $\mathsf{Verify}$, which says that $\mathsf{Verify}$ will accept any successful output of a $\mathsf{Find}$ operation.

Thus, if we make our simulator $\mathcal{S}_{\epsilon}$ nondeterministic, it can replace the $\mathsf{Find}$ operation by a nondeterministic guess as to the new value of the seed $\mathtt{Mem}$ during each phase, and then use $\mathsf{Verify}$ to check that this guess is correct, which requires only the $\mathcal{D}$ oracle. This immediately gives us a low space nondeterministic simulator $\mathcal{S}_{\epsilon}$ with similar properties as that in the proof of Theorem~\ref{lossy:thm:low-space-sim}. However, the total nondeterminism used by this simulation is $T(n)^{1 + O(\epsilon)}$, since it has to guess a seed of length $T^{O(\epsilon)}$ during each of the $T$ phases. 
...
\begin{theorem}\label{lossy:thm:full-ntisp-sim}
Let $T$ be an exponential time bound, and let $\mathcal{C},\mathcal{D}$ be a uniform compression scheme.
...
\subsection{Implications}\label{lossy:subsec:implications}

Consider the following three hypotheses:
\begin{hypothesis}\label{lossy:hyp:tisp-lb}
...
\]
\end{hypothesis}
\begin{hypothesis}\label{lossy:hyp:ntisp-lb}
There exists some exponential time bound $T$ and some $\epsilon > 0$ such that:
\[
...
\begin{hypothesis}\label{lossy:hyp:strong-ntisp-lb}
There exists some exponential time bound $T$ and some $\epsilon > 0$ such that:
\[
\cl{RAM\dash TIME}[T(n)] \not\subseteq \cl{1\dash NTISPG}[T(n)^{1+\epsilon}, T(n)^{\epsilon}, T(n)^{\epsilon}]
\]
...
While \ref{lossy:hyp:tisp-lb} and \ref{lossy:hyp:ntisp-lb} seem incomparable and both quite reasonable, we see that \ref{lossy:hyp:strong-ntisp-lb} is formally stronger then the previous two, and we have significantly less intuition as to whether or not it should be true. However, recall from Section~\ref{lossy:subsec:known-tradeoffs} that when the time bound $T$ is linear, all three of these hypotheses are known to hold unconditionally (indeed they hold for \emph{all} $\epsilon < 1$). In any case, the results of the previous section give us the following:
\begin{theorem}\label{lossy:thm:tisp-impl}
If Hypothesis~\ref{lossy:hyp:tisp-lb} holds, then for any uniform compression scheme $\mathcal{C},\mathcal{D}$ where both $\mathcal{C}$ and $\mathcal{D}$ are computable in polynomial time, there is a polynomial time algorithm which, given $1^n$, prints an incompressible string for $C_n,D_n$ for infinitely many $n$.
\end{theorem}
\begin{proof}
This follows from Theorem~\ref{lossy:thm:low-space-sim}; since the $\mathcal{C}$ and $\mathcal{D}$ oracle calls are of length $T(n)^{O(\epsilon)}$, if both $\mathcal{C}$ and $\mathcal{D}$ are computable in polynomial time then the oracles can be abandoned without effecting the resource bounds of the simulation.
\end{proof}
For Hypothesis 2, we have:
\begin{theorem}\label{lossy:thm:full-ntisp-impl}
If Hypothesis~\ref{lossy:hyp:ntisp-lb} holds, then for any uniform decompressor $\mathcal{D}$ computable in polynomial time, there exists there is a polynomial time \cl{NP}-oracle algorithm which, given $1^n$, prints an empty pigeonhole for $D_n$ for infinitely many $n$. In particular, $\cl{E}^\cl{NP} \not\subseteq \cl{size}[2^n/2n]$.
\end{theorem}
\begin{proof}
This follows from Theorem~\ref{lossy:thm:full-ntisp-sim}. As noted in Section~\ref{lossy:sec:pigeons}, any polynomial time decompressor $\mathcal{D}$ has a canonical ``proper compressor'' $\mathcal{C}$ computable with an \cl{NP} oracle, such that the incompressible strings for this scheme are empty pigeonholes for $\mathcal{D}$. The theorem then follows from the above proof, where we can abandon the $\mathcal{D}$ oracle in our simulation if $\mathcal{D}$ is computable in polynomial time since the oracle calls we make are small.
\end{proof}

Finally, the strongest hypothesis gives us the following:
\begin{theorem}\label{lossy:thm:swap-d-impl}
If Hypothesis~\ref{lossy:hyp:strong-ntisp-lb} holds, then for any uniform compression scheme $\mathcal{C},\mathcal{D}$ where $\mathcal{D}$ is computable in polynomial time, there is a polynomial time algorithm using a $\mathcal{C}$ oracle which, given $1^n$, prints an incompressible string for $C_n,D_n$ for infinitely many $n$.
\end{theorem}
\begin{proof}
This follows from Theorem~\ref{lossy:thm:swap-d-for-np}, where again we use the fact that $\mathcal{D}$ is computable in polynomial time to eliminate the $\mathcal{D}$ oracle.
\end{proof}

To illustrate the use of these theorems, we first consider their implications for the compression schemes related to non-uniform complexity measures described in Section~\ref{lossy:subsubsec:tt-gens}. In these cases, we have some complexity measure on strings/truth tables, such as circuit complexity, formula size, or $K^{poly}$ complexity, and wish to construct strings/truth tables of high complexity (for infinitely many input lengths), which we call the ``construction problem.'' In each case there is an associated ``compression problem'' in \cl{FNP}, where we are given a string/truth table and wish to find a small circuit/formula/program computing it if one exists. For these sorts of problems, we now have the following:
\begin{corollary}\label{lossy:cor:main-impls}
For each of the above mentioned uniform compression schemes related to non-uniform complexity measures, we have:
\begin{enumerate}
    \item If Hypothesis~\ref{lossy:hyp:tisp-lb} holds, then an efficient algorithm for the compression problem implies an efficient algorithm for the construction problem.
    \item If Hypothesis~\ref{lossy:hyp:ntisp-lb} holds, then there is an efficient \cl{NP}-oracle algorithm for the construction problem.
    \item If Hypothesis~\ref{lossy:hyp:strong-ntisp-lb} holds, then there is an efficient algorithm for the construction problem using an arbitrary oracle for the compression problem. In other words, the construction problem reduces to the compression problem.
\end{enumerate}
\end{corollary}
\begin{proof}
The first and second implications follow directly from the above theorems. For the last implication, there is a slight subtlety  related to the fact that the compression problem can generally have many valid solutions per instance, and so an oracle for this problem must be defined according to our notion of ``functional oracles'' (defined in Section~\ref{lossy:subsec:search-problems}). Thus, for a particular compression problem we have the following dichotomy for every one of its function oracles $\mathcal{O}$: either the compression scheme defined by $\mathcal{O}$ returns solutions that can be used as witnesses in the low space simulation for all but finitely many inputs, or else there is an explicit construction algorithm with access to $\mathcal{O}$ which prints strings of high complexity. So assuming Hypothesis~\ref{lossy:hyp:strong-ntisp-lb}, every $\mathcal{O}$ must fail to help the simulation for infinitely many inputs. Thus, given access to an arbitrary functional oracle for the compression problem, the explicit construction algorithm will work for infinitely many input lengths, although the choice of functional oracle might affect which infinite set of input lengths the algorithm works on.
\end{proof}
We see here that the conclusion of the third implication implies the conclusion of the previous two. As noted in the introduction, the second implication can be proved by simpler techniques, utilizing the ``Easy Witness Lemma.'' We give an alternate proof using this method in Appendix~\ref{lossy:appendix:ewl} for the interested reader. However, for the first and third implication, the J-tree update lemma seems necessary.

The case of large prime construction does not quite fit in with the others, as both the compressor and decompressor given in Theorem~\ref{lossy:thm:primes-in-apepp} require a factoring oracle. In addition, the second implication of Corollary~\ref{lossy:cor:main-impls} above is trivial when applied to the construction of large primes, since primality testing is in \cl{P} \cite{primes}. However we can still derive the following from Theorem~\ref{lossy:thm:tisp-impl}:
\begin{corollary}\label{lossy:cor:prime-construction}
If Hypothesis~\ref{lossy:hyp:tisp-lb} holds, then a polynomial time algorithm for factoring implies a polynomial time algorithm to construct $16n$-bit primes of magnitude $>2^n$ (for infinitely many $n$).
\end{corollary}

More generally we can conclude the following directly from Theorem~\ref{lossy:thm:low-space-sim}:
\begin{corollary}\label{lossy:cor:factoring-oracle}
One of the following is true:
\begin{enumerate}
    \item For every exponential time bound $T$ and every $\epsilon>0$, every language decidable in time $T(n)$ on a RAM machine can be decided in time $T(n)^{1+\epsilon}$ and space $T(n)^{\epsilon}$ by a 1-tape machine with a factoring oracle, which makes oracle calls of length at most $T(n)^{\epsilon}$.
    \item There is a polynomial time algorithm with a factoring oracle that generates $32n$-bit primes of magnitude $>2^n$ for infinitely many $n$.
\end{enumerate}
\end{corollary}
\section{BPP, TFNP, and the Weak Pigeonhole Principle}\label{lossy:sec:bpp}
Throughout this paper we have studied the problem of finding incompressible strings for \emph{uniform} compression schemes. As mentioned in Section~\ref{lossy:sec:pigeons}, it would also be natural to study the more general search problem, where a compression scheme of a fixed message/code length is given as input in the form of a boolean circuit:
\begin{definition}
...
\begin{theorem}\label{lossy:thm:bpp-pigeon}
...
\begin{corollary}\label{lossy:cor:yao}
...
\section{Succinct Representation of Primes}\label{lossy:appendix:primes}
We give here a proof of a generalization of Lemma~\ref{lossy:lem:divisibility-coding}.
\begin{lemma*}
...
\begin{theorem}\label{lossy:thm:3sat}
...
\end{lemma}
Recall that an almost identical theorem is proven in Chapter~\ref{chap:range} for the case where both inclusions are of the ``infinitely often'' type. We give a proof of this version below for completeness.
\begin{proof}
...
\end{enumerate}
The existence of a such a $D$ can follows directly from combining Lemmas \ref{lossy:lem:non-uniform-stretching} and \ref{lossy:lem:tt-in-apepp}. Now, for each $m \in \{\lceil \frac{6}{\epsilon} \rceil n, \ldots, \lceil \frac{6}{\epsilon} \rceil (n+1)\}$, we do the following:
\begin{enumerate}
    \item Compute the truth table of $L^m$, which takes time $2^{(c+1)m}$ with an \cl{NP} oracle.
    \item Attempt to find a seed $y \in \B^W$ such that for all $i \in \B^m$, $D^m[y](i) = b_i^W$ where $b_i$ denotes the $i^{th}$ bit of $L^m$. This can be accomplished using the $\mathsf{Iter\dash Compress}$ algorithm. By Lemma~\ref{lossy:lem:compress}, in $poly(|D|m) = O(2^{3n}n)$ time this procedure will either find such a $y$, or else find an empty pigeonhole $e$ for $D$. By construction of $D$, if we can find one of its empty pigeonholes this in turn allows us to construct a truth table of length $2^n$ and circuit complexity exceeding $2^n/2n$, also in $poly(2^n)$ time. If this happens, we halt the entire procedure and use this truth table to decide whether to accept or reject the input.
\end{enumerate}
