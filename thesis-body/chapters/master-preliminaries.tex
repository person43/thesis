...
We will delay most of the technical definitions we need until the chapters in which they are used, but cover here some basic definitions and conventions which will be used throughout this thesis.
\section{Basics}
...
\begin{definition}
We use $\mathsf{FP}$ to denote the set of search problems $R$ solvable by a polynomial time algorithm; more formally, there is a function $f$ solving $R$, such that $f(x)$ is computable from $x$ in $\poly(|x|)$ time. For a language $L$, we use $\cl{FP}^L$ (resp. $\cl{FP}^L_{\cl{\Vert}}$) to denote those search problems solvable in polynomial time with oracle access to $L$ (resp., in polynomial time with nonadaptive calls to an $L$ oracle). If $\cl{C}$ is a class with a polynomial time many-one complete problem, we write $\cl{FP^C}$ ($\cl{FP^C_{\Vert}}$) to denote $\cl{FP}^L$ (resp. $\cl{FP}^L_{\cl{\Vert}}$) for some complete problem $L$ (the definition is invariant to the choice of complete problem). 
...
Next, we define the notion of reducibility amongst search problems that will be used in this thesis:
\begin{definition}
Let $R_1, R_2$ be polynomially bounded search problems, and $\cl{C}$ a complexity class with a polynomial time complete problem. We say that $R_1$ is $\cl{FP^{C}}$ (resp. $\cl{FP^C_\cl{\Vert}}$) many-one reducible to $R_2$ if there is an $\cl{FP}$ function $f$ and a $\cl{FP^C}$ function (resp. $\cl{FP^C_\cl{\Vert}}$ function) $g$ so that the following holds: for any $x,y \in \B^*$, if $(f(x),y)\in R_2$ then $(x, g(x,y))\in R_1$.
\end{definition}

Finally, we recall our formal definition of an explicit construction problem:
\begin{definition}
...
We recall here the definition of Range Avoidance, which is also abbreviated ``\avoid:''
...
We make a note about the history of the name ``Range Avoidance.'' When the above search problem was originally introduced in \cite{KKMP}, it went under the name ``{\sc Empty};'' to be more precise, {\sc Empty} was used to refer to a more general (harder) search problem, of which Range Avoidance is a special case and refered to as ``{\sc 1-Empty}.'' In the followup work \cite{Korten21} this name persists, although the prefix 1 is dropped. Finally, in \cite{RSW}, the name Range Avoidance was coined, and since then it has become the accepted terminology, which we will use throughout this thesis. Finally, we note that in Chapter~\ref{chap:lop}, we will investigate the harder variant of \avoid first studied in \cite{KKMP} under the name ``{\sc Empty};'' in that chapter, we refer to the harder variant as ``Strong \avoid'' and the standard \avoid problem as ``Weak \avoid.''
