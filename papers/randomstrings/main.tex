...
\newcommand{\od}[1]{O^{\langle #1 \rangle}}
\newcommand{\lilod}[1]{o^{\langle #1 \rangle}}
\newcommand{\omd}[1]{\Omega^{\langle #1 \rangle}}
\newcommand{\semi}[2]{\Phi_{#1,#2}}
% \newcommand{\lsemi}[2]{\phi^{\langle #1 \rangle}_{#2}}
...
\title{How to Construct Random Strings}
\author{Oliver Korten\thanks{Department of Computer Science, Columbia University, New York, NY, USA. \href{mailto:oliver.korten@columbia.edu}{oliver.korten@columbia.edu}} ~and Rahul Santhanam\thanks{Department of Computer Science, Oxford University, UK. \href{mailto:rahul.santhanam@cs.ox.ac.uk}{rahul.santhanam@cs.ox.ac.uk}}}
% \author{Oliver Korten}{Department of Computer Science, Columbia University, New York, NY, USA}{oliver.korten@columbia.edu}

% \author{Rahul Santhanam}{Department of Computer Science, Oxford University, UK} {rahul.santhanam@cs.ox.ac.uk}
...
Under plausible complexity-theoretic assumptions, stating for example that there is an $\epsilon > 0$ for which $\TIME[T(n)] \not \subseteq \TIME^{\NP}[T(n)^{\epsilon}]/2^{\epsilon n}$ for appropriately chosen time-constructible $T$, we show that the answer to this question is positive (answering a question of \cite{RSW22}), and that the Range Avoidance problem \cite{KKMP21, Korten21, RSW22} is efficiently solvable for uniform sequences of circuits with close to minimal stretch (answering a question of \cite{ILW23}). 
...
\begin{hypothesis*}[Hardness Assumptions for $d^{th}$-Exponential Time Bounds]\label{hyp:intro}
~
\begin{enumerate}
...
\begin{theorem}[Informal, see Theorems~\ref{thm:elementary-generator} and \ref{thm:weak-generator}]
\label{thm:mainthm1}
...
\begin{theorem}[Informal, see Theorem~\ref{thm:elementary-strings-io}]
\label{thm:mainthm3}
...
In Section~\ref{subsec:ecs} we then use the previous results to give conditional polynomial time explicit constructions of various important combinatorial objects shown reducible to Range Avoidance in \cite{Korten21}. We will defer most of the specifics to Section~\ref{subsec:ecs}, but highlight the following two results:
\begin{theorem*}[See Theorems~\ref{thm:conditional-ecs-strong} and \ref{thm:conditional-ecs-weak}]
~
\begin{enumerate}
...
For a number $n \in \mathbb{N}$, we use $\K^{T,\mathcal{O}}(n)$ to denote $\K^T(\mathrm{bin}(n))$ where $\mathrm{bin}(n) \in \B^{\lceil \log n \rceil}$ is the canonical binary representation of $n$. 
\end{definition}
...
\begin{definition}[Explicit Construction Problems]
...
If $\Pi$ is an explicit construction problems, we say that a function $S: \B^* \to \B^*$ is a ``list solution'' to $\Pi$ if $S(1^n) \cap \Pi_n \neq \emptyset$ for all $n$. The ``list size'' $\ell(\cdot)$ is defined as $\ell(n) = |S(1^n)|$.
\end{definition}
As discussed in the introduction, an important class of explicit construction problems are those reducible to the problem \emph{range avoidance}:
\begin{definition}[Range Avoidance \cite{KKMP21,Korten21,RSW22}]
...
\end{definition}
% It was shown in \cite{Korten21} that a wide variety of important unsolved explicit construction problems can be reduced efficiently to range avoidance. Hence one approach to generically solving such problems is to find an efficient algorithm for the general range avoidance problem. Unfortunately, recent results \cite{ILW23} suggest that this is impossible. However, solving range avoidance in the worst case seems to significantly overshoot the goal of solving explicit construction problems: the key distinction, which we will address further in Section~\ref{subsubsec:uniform-vs-nonuniform}, is that to solve an explicit construction problem reducible to range avoidance, we only need to solve \emph{uniformly defined} instances of range avoidance, which is equivalent to producing strings of large $\K^{\poly(n)}(\cdot) $ complexity:
...
\begin{observation}[\cite{RSW22}]\label{obs:avoid-to-random-strings}
Say that for every $k \in \mathbb{N}$, there is a polynomial time algorithm which, for every $n$ (resp. infinitely many $n$) outputs a string $x \in \B^n$ with $\K^{n^k}(x) \geq \ell(n)$. Then every explicit construction problem reducible to Avoid with stretch function $\ell(n)$ is solvable in polynomial time .
\end{observation}
\begin{proof}
The definition of the reduction implies that there is a polynomial time algorithm $C_n: \B^{\ell(n)} \to \B^n$, so that whenever $x \notin \mathrm{range}(C_n)$ we have $x \in \Pi$. If $k$ is such that the algorithm constructing $C_n$ runs in time $n^k$, then we observe that every string $x$ in the range of $C_n$ satisfies $\K^{n^k}(x) \leq \ell(n)$.
\end{proof}


% Hence, if our goal is to construct strings with large $\K^\poly$ complexity in polynomial time, it would be sufficient to find polynomial time algorithms for range avoidance. Unfortunately this approach seems unlikely to be realized, since the general range avoidance problem is now known to be hard under widely-believed cryptographic assumptions {\color{red}cite}. Fortunately we may observe that the reduction in Observation~\ref{obs:avoid-to-random-strings} overshoots the more conservative goal we have laid out {\color{red} previously in intro}, which is to construct strings with large \emph{unconditional} Kolmogorov complexity, i.e. $\K^\poly(x)$ as large as possible. For this purpose we have the following approach available:
...
\begin{observation}\label{obs:concat}
...
\begin{observation}\label{obs:prg-avoid}
...
\section{PRGs for Uniform Classes with Near-Optimal Seed Length}\label{sec:PRG}
...
\begin{hypothesis}[Strong Assumption for $\mathcal{O},d$, abbreviated $\SH(\mathcal{O},d)$]\label{hyp:strong}
...
\begin{hypothesis}[Weak Assumption for parameters $\mathcal{O},d,v$, $d \geq 1$, abbreviated $\WH(\mathcal{O},d,v)$]\label{hyp:weak}
...
\begin{lemma}\label{lem:polytime-prg-from-compression}
...
\end{lemma}
\begin{lemma}\label{lem:weak-prg-from-hardness}
...
\end{lemma}
We will prove the first case (Lemma~\ref{lem:polytime-prg-from-compression}) here, and reserve Lemma~\ref{lem:weak-prg-from-hardness} for the Appendix as it's proof is similar. We require the following standard tool from complexity-theoretic pseudorandomness:
\begin{theorem}[Black-Box Hardness-Randomness Connection \cite{IW97}]\label{thm:IW-generalized}
...
\begin{proof}[Proof of Lemma~\ref{lem:polytime-prg-from-compression}]
...
% \begin{lemma}\label{lem:compression-hardness-rand}
...
% \end{lemma}
% \begin{proof}
% Let $\mathcal{O}$ $\epsilon$ be as above and let $T$ be the time bound which we want to fool. Invoke Theorem~\ref{thm:IW-generalized} for $\epsilon/2$ and let $c \in \mathbb{N}$ be the guaranteed constant. 

% \end{proof}


% \begin{hypothesis}[Assumption $(T,B,\mathcal{F})$:]\label{hyp:main}
...
% \begin{lemma}
% Assume Hypothesis~\ref{hyp:main} for $T,B$
% \end{lemma}
% \begin{theorem}
...
% \begin{theorem}\label{thm:elementary-generator}
% Let $\mathcal{O} \subseteq \B^*$ be any oracle, $d, c\in \mathbb{N}$, and assume Hypothesis~\ref{hyp:elementary} for the oracle $\mathcal{O}$ and the constant $d+v+1$. Let $T(n) = \exp^{[d]}(n)$. There exists a generator $(\mathcal{G}^d_n: \B^{s_d(n)} \to \B^n)_{n \in \mathbb{N}}$ computable uniformly in $T(O(n)) = \od{d+1}(T(n))$ time \todo{verify}, so that the following hold:
% \begin{enumerate}
...
% \end{theorem}
% \begin{proof}
% Using our hardness assumption in combination with Lemma~\ref{lem:timescaled-hardness-rand}, we conclude that for every $a \in \mathbb{N}$ there is $c_a \in \mathbb{N}$ and a generator $(G^{a}_n: \B^{c_a \lceil \log n \rceil } \to \B^n$ computable uniformly in time $\exp^{[a]}(O(n))^{c_a}$ time which fools $\TIME^{\mathcal{O}}[\exp^{[a]}(n)]/3n$ for all sufficiently large $n$. 
...
% \begin{hypothesis}\label{hyp:computable}
...
% \begin{theorem}\label{thm:inverse-computable-generator}
% Assume Hypothesis~\ref{hyp:computable}. Let $f(n): \mathbb{N} \to \mathbb{N}$ be a total computable function which grows without bound, and whose functional inverse is computable in polynomial time. There exists a generator $\mathcal{G}_n: \B^{s(n)} \to \B^n$ computable uniformly in $T(n)^{O(1)}$ time, so that the following hold:
% \begin{enumerate}
...
% \end{theorem}
% \begin{proof}
% The proof is at a high level identical to that of Theorem~\ref{thm:elementary-generator}; we will only indicate those parts which need to change. At the start we will set $\lambda = \log \K^{T(n)}(n) + a(n)$ as before. Fix $c$ to be a sufficiently large constant depending on the runtime exponent and hardness parameter $\epsilon>0$ for the language $L$ in Hypothesis~\ref{hyp:computable}. As in Theorem~\ref{thm:elementary-generator} we define $T^0 = T$, $T^{i} = T(\exp^{[i]}(\cdot))^{c + 1}$, $n_{i+1} = c_i \cdot \lceil \log n_i \rceil $ {\color{red} correct this}
...
\begin{theorem}\label{thm:double-exp-generator}
...
\end{theorem}
\begin{proof}
We will set $v=0$, $d = 1$, $T(n) = n^k$ in Theorem~\ref{thm:elementary-generator}. Observe that for all $n$, we have $\K^{O(n)}(n) \leq \log n$ since we may encode the number $n$ in binary.
\end{proof}

If we rely instead on the weak form hypotheses $\WH(\cdot)$ we get:

\begin{theorem}\label{thm:weak-generator}
...
% Assume Hypothesis~\ref{hyp:weak} for oracle $\mathcal{O}$ and constant $d+1$, and let $T(n) \leq (\exp^{[d]}(n))^{O(1)}$ be time constructible. There exists a generator $(\mathcal{G}_n^d: \B^{s_d(n)} \to \B^n)_{n\in \mathbb{N}}$ computable uniformly in $\od{2d+2}(T(n))$ time \todo{check runtime}, so that the following hold:
% \begin{enumerate}
...
As in the case of Lemma~\ref{lem:weak-prg-from-hardness}, we relegate the proof to the appendix since it uses essentially the same ideas as that of Theorem~\ref{thm:elementary-generator} and is in fact a bit simpler. As before we highlight an important special case, obtained immediately from Theorem~\ref{thm:weak-generator} by setting $\mathcal{O}= \NP$, $d=1$, $v = 2$:

\begin{theorem}\label{thm:weak-loglog-gen}
...
\subsection{Fooling Uniform Deterministic Time }\label{subsec:deterministic}
...
We start by applying our PRG constructions to give polynomial time explicit constructions of highly random strings. The first is a direct combination of our main PRG constructions with Observations~\ref{obs:concat} and \ref{obs:prg-avoid} from the preliminaries. We start with the situation in which we use the strong form of our hardness assumptions, and hence Theorem~\ref{thm:elementary-generator} as our PRG:
\begin{theorem}\label{thm:elementary-random-strings-ae}
...
\end{theorem}
\begin{proof}
This is a rather direct corollary of Theorem~\ref{thm:elementary-generator}. Let $n$ be given with $\K^{T(n)}(n) \leq \log^{[d]}(n)$, and let $s_d(\cdot) \leq O(\log^{[d+1]}(\cdot))$ be the seed length bound of the generator $\mathcal{G}^d$ from Theorem~\ref{thm:elementary-generator} using the base time bound $T(n)$. Let $n'$ be the largest integer such that $n'2^{s_d(n')} \leq n$; clearly $\K^{T(n)}(n') \leq \K^{T(n)}(n) + O(1) \leq \log^{[d]}(n)$, hence we may apply Theorem~\ref{thm:elementary-generator} to compute (uniformly) a pseudorandom generator $\mathcal{G}^d_n: \B^{s_d(n')} \to \B^{n'}$ which fools $\TIME^{\NP}[T(n)]/O(1)$ on input length $n'$. By Observation~\ref{obs:prg-avoid} we conclude that some string $z$ in the range of $\mathcal{G}_n$ has $\K^{T(n)}(z) \geq n'-1$, hence the concatenation of all strings in the range of $\mathcal{G}^d_n$ (padded at the end with $0$s to bring the length up to $n$) will have $\K^{T(n)}(\cdot)$ complexity at least $n' - s_d(n') \geq \frac{n}{(\log^{[d]}(n))^{O(1)}}$ (recall Observation~\ref{obs:concat}). 

The second part of the theorem is the special parameter setting given by Theorem~\ref{thm:double-exp-generator}, and for the last sentence in the statement, we use the fact that Theorem~\ref{thm:double-exp-generator} works for any oracle $\mathcal{O'}$.
\end{proof}

If we instead rely on the weaker form hardness assumptions and the associated PRG from Theorem~\ref{thm:weak-generator} we obtain via the exact same argument:
\begin{theorem}\label{thm:elementary-random-strings-weak}
...
\begin{theorem}\label{thm:elementary-strings-io}
...
\subsection{Explicit Constructions Under Plausible Hardness Assumptions}\label{subsec:ecs}
...
\begin{definition}[List of Explicit Construction Problems]\label{def:list-of-problems}
~
\begin{itemize}
...
Say that a Range Avoid instance has ``stretch $n \mapsto m$'' if it is of the form $C: \B^n \to \B^m$. The following uniform reductions exist from the problems in Definition~\ref{def:list-of-problems} to Range Avoidance:
\begin{enumerate}
...
Combining this with our first generator construction from the previous section and Observation~\ref{obs:avoid-to-random-strings}, we have:
\begin{theorem}\label{thm:conditional-ecs-strong}
...
We arrive at similar conclusions if we use the weak form hypothesis in combination with Theorem~\ref{thm:weak-generator} instead, at the cost of a slow-down in our construction algorithms. We highlight below the results obtained by using the special case of Theorem~\ref{thm:weak-generator} given in Theorem~\ref{thm:weak-loglog-gen}:
\begin{theorem}\label{thm:conditional-ecs-weak}
...
% Let $t$ be such that $t(2^n) = T(n)^{1/c}$, for a constant $c > 1$ that we will soon specify. Note that since $T$ is at least exponential, $t$ is at least linear. We apply the last part of Theorem \ref{thm:elementary-random-strings-ae} to $t$ with the $\mathsf{QBF}$ oracle, yielding an polynomial-time algorithm $\mathcal{A}$ that on input $1^n$, outputs a string $y_n$ of length $n$ with $\K^{\mathsf{QBF},t}$ complexity at least $n/\polylog(n)$. Let $c$ be a constant upper bound on the exponent of the running time of the polynomial-time algorithm $\mathcal{A}$. Note that the assumption required for this in Theorem \ref{thm:elementary-random-strings-ae} is for a time bound double exponential in $t$, which is elementary and time-construtible, therefore the first item suffices for this. 
...
\begin{proof}
We show that we can take $c$ to be $1+\delta$, for arbitrarily small $\delta > 0$. Indeed, for such $c$ and $d \geq c$, the intractability of general Range Avoidance follows from Theorem \ref{t:ILW}, under the given assumptions. We show that uniform Range Avoidance can be solved by applying Theorem \ref{thm:elementary-random-strings-ae} and using the third assumption. Indeed, by this assumption, for every constant $k$, there exists a polynomial-time algorithm $A_k$ which for all $n$, on input $1^n$ outputs a length-$n$ string of $\K^{n^k}$ complexity at least $n/\polylog(n)$.
...
\begin{proof}
For the negative result on Range Avoidance, we simply apply Theorem \ref{t:PSPACERangeAvoid}. For the positive result, we apply Theorem \ref{thm:elementary-random-strings-ae} (relativized to a $\mathsf{TQBF}$ oracle) and then use the same argument as in the proof of the previous corollary.
\end{proof}

\section{Barriers to Improvements}
In this section we present two barriers to improving our main results. First we show that the seed length $\log^{[O(1)]}n$ achieved in our PRG from Section~\ref{sec:PRG} is in some sense optimal for hardness/randomness approaches which use the same overall structure as our argument: specifically, we show that our PRG construction corresponds to a construction of a special kind of seeded extractor with seed length $\log^{[O(1)]}n$, and prove unconditionally that this is the minimal achievable seed length in any such construction. Second we show that there is no relativizing argument that directly reduces the construction of $\K^{\poly(n)}$-random strings to the construction of hard truth tables.
...
In Section~\ref{sec:PRG} a PRG was constructed against \emph{uniform} algorithms (or more generally, algorithms with extremely small advice), whose seed length was much smaller than $\log n$, using a different kind of hardness assumption applied across many different input lengths. In the section we cast such a hardness/randomness construction in terms of certain seeded \emph{multi-source} extractors, where each source corresponds to a hardness assumption used at a different input length. Through this connection we are able to show that the iterated logarithmic ($\log^{[d]}(n)$, $d= O(1)$) seed length in our PRGs from Section~\ref{sec:PRG} is necessary for any hardness/randomness tradeoff that uses the same general framework as ours, and in particular which uses ``only $O(1)$ many'' hardness assumptions.

We start by recalling the proof of correctness of the PRG from Section~\ref{sec:PRG}, first in the case where $O(\log \log n)$ seed length is achieved using a hardness assumption at two different input lengths. We first used a hard function whose truth table had size $\poly(n)$ in order to get a PRG with seed length $O(\log n)$; call this $f_1$. We then bounded the run time of this PRG by some $T(n) = \poly(n)$, and in the next step required a hard function $f_2$ of truth table size $\approx \log n$, which was hard for algorithms with running time $T(n)$; in particular $f_2$ needed to be hard even in the regime where the computation of $f_1$ is easy. For this reason, we may roughly interpret this as saying, $f_2$ is a function (of much smaller input length) which is hard \emph{conditioned on $f_1$}. 
...
\begin{lemma}\label{lem:recon-to-CG}
...
\end{proof}
We can now recast the central step in our main construction in Theorem~\ref{thm:elementary-generator} in the language of reconstructive extractors:
\begin{theorem}[Essentially in \cite{NZ}]
Let $E^1,\ldots,E^d$ be single source extractors $E^j: \B^{m_j} \times \B^{s_j} \to \B^{n_j}$, so that $E^j$ is a reconstructive single-source extractor for min entropy $k_j$ and error $\epsilon_j$ and is computable in polynomial time. Say that for each $j < d$, we have $s_j = n_{j+1}$. Define $\tilde{E}^j: \prod_{j' \leq j} \B^{m_{j'}} \times \B^{s_j} \to \B^{n_1}$ by induction on $j$, with $\tilde{E}^1 = E^1$, and $\tilde{E} ^{j+1}(x_1,\ldots,x_{j+1},z) = \tilde{E}^{j-1}(x_1,\ldots,x_{j-1}, E^j(x_j, z))$. Then $\tilde{E}^d$ is a multi-reconstructive extractor for entropy sequence $(k_1,\ldots,k_d)$ and error $\sum_{j \leq d} \epsilon_d$. 
\end{theorem}
In \cite{NZ}, the same method of composing single source extractors to form a seeded block source extractor is analyzed in the information theoretic setting (rather than the computational/reconstructive setting). The proof of this lemma in the reconstructive framework is essentially identical to the core argument justifying the correctness of our pseudorandom generator construction in Theorem~\ref{thm:elementary-generator}:
...
Now, say that $\tilde{E}^j$ is a reconstructive extractor for entropy sequence $(k_1,\ldots,k_j)$ and error $\delta_j$. So there are reconstruction procedures $R_1,\ldots,R_j$ taking oracle access to a function $D: \B^{n_1} \to \B$ so that, given any $D \subseteq \B^n$ distinguishing $\tilde{E}^{j}(\bar{x}, z)$ with error $\delta$, there exists $j' \leq j$ so that $R_{j'}^D(x_,\ldots,x_{j'-1}, a)$ produces $x_{j'}$ for some $a \in \B^{k_{j'}}$. We then consider $\tilde{E}^{j+1}(x_1,\ldots,x_{j+1},z')$; our reconstruction procedures for $R_1,\ldots,R_j$ will be as they were for $\tilde{E}^{j+1}$, and for $R_{j+1}$ we use the construction procedure for the single source extractor $E^{j+1}$. 
Let $D$ be given. For any $x_1,\ldots,x_{j+1}$ such that the first $R_1,\ldots,R_j$ reconstruction procedures all fail to reconstruct a symbol of $x_1,\ldots,x_{j+1}$ from its prefix using $D$, we know that $\tilde{E}^{j}(x_1,\ldots,x_{j+1}, \mathcal{U}_{s_j})$ fools $D$ with error $\delta_j$. Now define the test $D' \subseteq \B^{n_{j+1}}$ (recall $n_{j+1} = s_j$), with $D'(z)= D(\tilde{E}^{j}(x_1,\ldots,x_{j}, z))=1$. Then, for a uniformly random $z \in \B^{s_j}$ with have that $D'(z) = 1$ with probability in the range $\Pr_{y \sim n_{j+1}}[D(y)=1] \pm \delta_j$. Observe that $D'$ is efficiently computable with oracle access to $D$ and the values $x_1,\ldots,x_j$, since each of the $E^{j'}$ extractors are efficiently computable. Thus, using the reconstruction procedure for $E^{j+1}$, either $R_{j+1}$ succeeds in reconstructing $x_{j+1}$ using oracle access to $D$, the previous inputs $x_1,\ldots,x_j$, and $k_{j+1}$ bits of advice, or else we must have that $E^{j+1}(x_{j+1}, \mathcal{U}_{s_{j+1}})$ fools $D'$ with error $\epsilon_{j+1}$, and hence the overall construction fools $D$ with error $\delta_j + \epsilon_{j+1}$, completing the proof.
\end{proof}
...
\]
Moreover, $k_j \geq \log^{[j]}n$ for each $j$, and hence by Lemma~\ref{lem:recon-to-CG} this construction is also a block source extractor with the same parameters stated above.
\end{corollary}
...
\begin{theorem}\label{thm:seed-length-lb}
...
\begin{lemma}\label{lem:CG-chain-rule}
...
\end{lemma}
\begin{proof}[Proof of Lemma~\ref{lem:CG-chain-rule}]
We prove by induction on $d$; the case $d=1$ is trivial. Let $A$ be given; for each $x_1 \in \B^{m_1}$ define $A_{x_1} = \{(x_2,\ldots x_d) \mid (x_1,x_2,\ldots,x_d) \in A\}$, $\mathrm{deg}(x_1) = |A_{x_1}| $. So $\sum_{x_1 \in \B^{m_1}} \deg(x) = |A|.$
...
To argue that $m_j \leq 2^{s+2} + 2d$ for some $j$ we rely on Lemma~\ref{lem:CG-chain-rule} above. Let $\mathcal{D} \subseteq \B^{\B^n}$ consist of all functions $D: \B^n \to \B$ which depend only on the first $s+1$ bits. So $|\mathcal{D}| \leq 2^{2^{s+1}}$. For every $\bar{x} \in \prod_{j \leq d} \B^{m_j}$, there is some $D_{\bar{x}} \in \mathcal{D}$ such that 
\[
...
\begin{theorem}\label{thm:tt-K-equiv}
The following are equivalent:
\begin{enumerate}
...
We show here that there is no such reduction which is ``black box'' in the same sense as Theorem~\ref{thm:tt-K-equiv}. We first need to define the suitable notion of a black box reduction:
\begin{definition}
...
% For any time constructible $T(n)$ there exists a uniform deterministic algorithm running in time $T^{1+o(1)}(n)$ producing a string $x \in \{0,1\}^n$ with $\mathsf{K}^{T}(x) \geq n-1$ 
% \end{definition}
...
\appendix
\section{Proof of Lemma~\ref{lem:weak-prg-from-hardness}}
We prove here Lemma~\ref{lem:weak-prg-from-hardness}, which we restate below for convenience:

% \begin{lemma*}[Lemma~\ref{lem:weak-prg-from-hardness}, Restated]
% Let $d \geq 0$, $\mathcal{O} \subseteq \B^*$. Assume Hypothesis~\ref{hyp:weak} for oracle $\mathcal{O}$ and constant $d+1$. Then for every time constructible $T(n) \leq \poly((\exp^{[d]}(n))$ there is a PRG $(G_n: \B^{s(n)} \to \B^n)_{n \in \mathbb{N}}$ computable uniformly in time $\od{2d+2}(T(n))$ which fools $\TIME^{\mathcal{O}}[T(n)]/3n$ and has seed length $s(n) \leq \od{d+1}(\log n)$. 
% \end{lemma*}
...
For this we require a strengthening of Theorem~\ref{thm:IW-generalized} due to Umans \cite{umans} (following a similar result of Shaltiel-Umans \cite{shaltiel-umans}) given as follows:

\begin{theorem}[\cite{umans}]\label{thm:umans}
...
We will use a slight generalization of Lemma~\ref{lem:weak-prg-from-hardness} that follows directly by padding (the difference compared to the previous lemma is merely that we allow a slightly more general upper bound on $T(n)$):
\begin{corollary}\label{cor:scaled-weak}
