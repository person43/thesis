...
\newcommand{\N}{[N]}
\newcommand{\xnote}[1]{\footnote{{\color{purple}Toni}: {#1}}}
\usepackage[numbers,sort&compress]{natbib}
\def\bibfont{\footnotesize}
\def\bibsep{\smallskipamount}
\def\bibhang{24pt}
...
\def\cl{\bf}
\author{
Oliver Korten\textsuperscript{$\dagger$,$\S$} \qquad
Toniann Pitassi\textsuperscript{$\ddagger$,$\S$} \qquad
Russell Impagliazzo\textsuperscript{$\mathparagraph$,$\S$}
}

\date{}
\title{Stronger Cell Probe Lower Bounds via Local PRGs}
\begin{document}
\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\footnotetext[1]{Department of Computer Science, Columbia University. \href{mailto:oliver.korten@columbia.edu}{oliver.korten@columbia.edu}}
\footnotetext[2]{Department of Computer Science, Columbia University. \href{mailto:tonipitassi@gmail.com}{tonipitassi@gmail.com}}
\footnotetext[3]{Department of Computer Science, University of San Diego.  \href{mailto:rimpagliazzo@ucsd.edu}{rimpagliazzo@ucsd.edu}}
\footnotetext[4]{Supported by NSF AF:Medium 2212136.}
...
\begin{abstract}
In this work we observe a tight connection between three topics: $\mathsf{NC}^0$ cryptography, $\mathsf{NC}^0$ range avoidance, and static data structure lower bounds. Using this connection, we
leverage techniques from the cryptanalysis of $\mathsf{NC}^0$ PRGs to prove state-of-the-art results in the latter two subjects. Our main result is an improvement to the best known static data structure lower bounds, breaking a barrier which has stood for several decades.
Prior to our work, the best known lower bound for any explicit problem with $M$ inputs and $N$ queries was ${S \geq N^{\frac{1}{t}} (\log M)^{1-\frac{1}{t}}}$ for any setting of the word length $w$ (where $S=$~space and $t=$~time) \cite{Siegel89}. We prove, for the same class of explicit problems considered in \cite{Siegel89}, a quadratically stronger space lower bound of the form $S \geq \tilde{\Omega}\Bigl( N^{\frac{2}{t}} \cdot (\log M)^{1-\frac{2}{t}} \cdot 2^{-O(w)} \Bigr)$ for all even $t>0$. Second, for the restricted class of \emph{nonadaptive bit probe} data structures, we improve on this lower bound polynomially: for all odd constants $t>1$ we give an explicit problem with $N$ queries and $M \leq N^{O(1)}$ inputs and prove a lower bound $S \geq \Omega(N^{\frac{2}{t} + \epsilon_t})$ for some constant $\epsilon_t > 0$. Our results build off of an exciting body of work on refuting \emph{semi-random} CSPs (e.g., \cite{semirandom,smoothed,simpler-moore}).

We then utilize our explicit cell probe lower bounds to obtain the best known unconditional algorithms for $\mathsf{NC}^0$ range avoidance: we can solve any instance with stretch $n \mapsto m$ in polynomial time once $m >> n^{\frac{t}{2}}$ when $t$ is even; with the aid of an $\mathsf{NP}$ oracle we can solve any instance with $m > n^{\frac{t}{2} - \epsilon_t}$ for $\epsilon_t>0$ when $t$ is odd. Finally, using our main correspondence we establish some barrier results for obtaining significant improvements to our cell probe lower bounds: (i) near-optimal space lower bounds for an explicit problem with $t=4, w=1$ implies 
$\mathsf{EXP^{NP}} \not\subseteq \mathsf{NC}^1$; (ii) under the widely-believed assumption that polynomial-stretch $\mathsf{NC}^0$ PRGs exist, there is no \emph{natural proof} of a lower bound of the form $S \geq N^{\Omega(1)}$ when $t = \omega(1)$, $w=1$.
...
\begin{gather}
  S \geq N^{\frac{1}{t}} \cdot (\log M)^{1 - \frac{1}{t}}   \label{eq:1}
\end{gather}
...
We emphasize that an analogous connection exists for the general cell probe model (adaptive and with arbitrary word length), which is explained formally in Section~\ref{subsec:gen-vs-cell}.
...
The connection between $\mathsf{NC}^0$-PRGs and cell probe lower bounds is more difficult to state formally, but even more central to our main results. In one direction, we will show how to use the best known methods for distinguishing $\mathsf{NC}^0$ PRGs to achieve state of the art lower bounds in the cell probe model; this is the most significant contribution in this work, and after stating our main results below we will describe this methodology in more detail in Section~\ref{subsec:techniques} (``Our Techniques'').
...
We emphasize that, in addition to giving a quadratic space improvement over the \emph{cell sampling} bound (the first improvement of any kind), our lower bound is also the first to achieve any polynomial improvement over the much simpler \emph{communication bound} $S \gtrsim N^{\frac{1}{t}}$ for a problem with $N$ queries and $M \leq \exp(N^{o(1)})$ datapoints (by setting $k=\log N$ in the above); this is discussed further in the Section~\ref{subsec:prior}. 

By known constructions \cite{Siegel89,Siegel04} (see also \cite{pudlak}), the lower bound in Theorem~\ref{thm:k-wise-lb} is tight up to $\log N$ factors and the factors depending only on $w$ provided $t$ is not too large. Hence, for all small enough even values of $t$ and small enough $w$, we essentially completely resolve the cell probe complexity of representing $k$-wise independent distributions. The almost-matching upper bounds are nonconstructive, relying on unbalanced bipartite graphs with very strong unique-expansion properties; they yield data structures which are nonadaptive and have word length $w=1$, so the tightness of Theorem~\ref{thm:k-wise-lb} holds even in this restricted setting of nonadaptive bit probe data structures.

Our lower bound for $k$-wise independent problems is actually a bit stronger than stated above, as we are able to establish the same bound for problems which only exhibit $\gamma$-\emph{almost} $k$-wise independence, for a nontrivially large value of $\gamma$. The stronger form (which is the form which will appear as Theorem~\ref{thm:k-wise-lb} in Section~\ref{subsec:k-wise-lbs}) is as follows:
\begin{theorem*}[Theorem~\ref{thm:k-wise-lb}, Strengthened To Almost Independence]
% \label{thm:almostk-wise-lb}
%[Theorem~\ref{thm:k-wise-lb}, Strengthened to Almost-Independence]
...
Then the same lower bound as in the first presentation of Theorem~\ref{thm:k-wise-lb} (above) holds for $F$ provided $\gamma \leq  N^{t} 2^{-O(ktw)}$.
\end{theorem*}
...
We emphasize that, because of the optimality of the bound in Theorem~\ref{thm:k-wise-lb} (there is an upper bound almost matching the bound in Theorem~\ref{thm:k-wise-lb} even in the nonadaptive bit probe setting and even for odd values of $t$), the above Theorem~\ref{thm:biased-lb} is provably unattainable using \emph{only limited independence}. More formally, to construct any data structure problem $F: [N] \times [M] \to \B$ satisfying a lower bound $S \geq N^{\frac{2}{t} + \epsilon}$ for any constants $t > 1$, $\epsilon>0$, and $M \leq \exp(N^{o(1)})$, we provably cannot use $k$-wise independence as our only hardness property for the problem $F$. This result is therefore of special interest, as it the first example of a cell probe lower bound that is provably stronger than the best bound provable using limited independence alone: all of the previous best lower bound methods \cite{Siegel89,panigrahy,kasper,viola-maximal,golovnev}\footnote{Viola's lower bound for $t=2$ nonadaptive bit probe data structures \cite{viola-maximal} does no use $k$-wise independence, but $k$-wise independence was shown to achieve the same quantitative lower bound in the follow up work of \cite{golovnev}.} can be carried out using limited independence as the sole hardness condition.
~\\~\\
{\bf \noindent $\mathsf{NC}^0$ Range Avoidance Algorithms:}
In Section~\ref{sec:avoid-and-linear-attack} we give the best known unconditional algorithms for $\mathsf{NC}^0$ range avoidance. Using our primary observation connecting range avoidance to data structure lower bounds together with known constructions of limited independence and small bias spaces \cite{naornaor}, we can immediately obtain state of the art $\mathsf{P^{NP}}$ algorithms for $\mathsf{NC}^0$ range avoidance using our new cell probe lower bounds as a black box. In the case of Theorem~\ref{thm:biased-lb} we get:

\begin{theorem}
\label{thm:pnp-avoid-odd}
%[Theorem~\ref{thm:pnp-avoid-odd}]
Let $t = O(1)$ be a fixed odd number. There is a polynomial time algorithm which outputs a list of strings in $\{0,1\}^m$, such that for any $\mathsf{NC}^0_t$ generator $G: \B^n \to \B^m$ with
\[
...
For our lower bound in Theorem~\ref{thm:k-wise-lb}, we are able to analyse it more effectively (rather than applying it as a black box) and remove the use of an $\mathsf{NP}$ oracle in the associated range avoidance algorithm:

\begin{theorem}
\label{thm:polytime-avoid}
%[Theorem~\ref{thm:polytime-avoid}]
...
% \begin{theorem}
% \label{thm:explicit-vs-polystretch}
...
% \end{theorem}
\begin{theorem}[Consequences of Explicit Data Structure Lower Bounds]\label{thm:explicit-vs-avoid}
~
\begin{enumerate}
...
% The circuit lower bounds in the conclusions of \cite{viola-maximal} and Theorem~\ref{thm:explicit-vs-avoid} are also apparently incomparable.
...
We argue in Section~\ref{subsec:natural-vs-prg} that the strongest lower bound methods in the cell probe model known prior to this work can be made \emph{natural} in this sense. This continues to hold for our new lower bound in Theorem~\ref{thm:k-wise-lb}.
...
\label{thm:nat-prop-barrier}
%[Theorem~\ref{thm:nat-prop-barrier}]
~
\begin{enumerate}
...
As mentioned above our lower bound in Theorem~\ref{thm:k-wise-lb} can be made natural, as well as all lower bounds proven prior using the cell sampling method or communication arguments. However, interestingly, we \emph{do not} know if the lower bound for small bias distributions in Theorem~\ref{thm:biased-lb} can. 
...
\end{enumerate}
In both cases, it is required as in Theorem~\ref{thm:biased-lb} that $t=O(1)$ is an odd constant.

\subsection{Our Techniques: Derandomizing Semirandom CSP Refutations}\label{subsec:techniques}

We now discuss at a high level our approach to proving our main cell probe lower bound for $k$-wise independent problems (Theorem~\ref{thm:k-wise-lb}). We will narrow down our discussion in three respects: (1) we focus on the case $t= O(1)$ and ignore multiplicative factors depending on $t$ in our space lower bound, (2) we restrict attention to \emph{nonadaptive bit probe} data structures, and (3) we focus on achieving a lower bound of the form $S \geq \tilde{\Omega}(N^{\frac{2}{t}})$ for $k$-wise independent problems in the special case $k = O(\log N)$.
...
\begin{enumerate}
    ...
    \]
    is small. The reader unfamiliar with the $\infty\to1$ norm should think of it for now as close relative of the spectral norm which can be approximated in polynomial time (a formal definition is in Section~\ref{subsec:basic-prelims}).
...
\begin{problem}\label{prob:odd-vs-even}
Extend the bound in Theorem~\ref{thm:k-wise-lb} to hold for odd values of $t$, and the bound in Theorem~\ref{thm:biased-lb} to hold for even values of $t$.
\end{problem}
 The natural approach here is to utilize the CSP refutation procedures for odd order CSPs developed in \cite{semirandom,smoothed} and analyze them in the ``pseudorandom case'' as we have done for even order CSPs. It seems likely that a solution to the first part of Problem~\ref{prob:odd-vs-even} would automatically solve the second part (see the discussion at the end of Section~\ref{sec:nonadaptive-lbs}).

Our second problem relates to Theorem~\ref{thm:biased-lb}. While our lower bounds for $k$-wise independent data structure problems are known to be optimal up to lower order terms (for even $t$), our nonadaptive bit probe lower bounds for $\epsilon$-biased distributions have no matching upper bound. We highlight closing this gap as an important open problem:
...
\end{problem}
Our Theorem~\ref{thm:biased-lb} yields the lower bound $\delta_t\geq \Bigl( \frac{t}{2} - \frac{t-2}{2(t+2)}\Bigr)^{-1}$, in particular $\delta_t > \frac{2}{t}$, for every odd value $t \geq 3$. Theorem~\ref{thm:k-wise-lb} (the second form described above) also implies $\delta_t \geq \frac{2}{t}$ for even values of $t$ since $\epsilon$-almost $k$-wise independence is a special case of $\epsilon$-biasedness; this latter lower bound holds in the more general adaptive setting and with larger word lengths. Both of these lower bounds hold even if we only aim to achieve bias $\poly(N)^{-1}$ (rather than $\exp(-N^{o(1)})$ as stated above). On the other hand a construction of \cite{mossel-epsilon} implies that $\lim\limits_{t \to \infty} \delta_t = 0$, in particular $\delta_t \leq O(\sqrt{\frac{1}{t}} )$. Closing this gap in either direction remains an intriguing open problem.

% Second, we highlight the problem of replacing the term $\frac{N}{\log N}$ by $\Omega(N)$ in the setting $k = O(\log N)$ of Theorem~\ref{thm:k-wise-lb}, possibly using a different hard problem $F$:
...
% \end{problem}
% Note that Theorem~\ref{thm:biased-lb} holds only for nonadaptive data structures and hence does not answer this question. 
...
% Nonetheless it is conceivable that one or both parts of Problem~\ref{prob:constructive-moore} could be solved unconditionally by a different approach.
A different approach, tailored specifically to (b), is to show that the lower bound in Theorem~\ref{thm:biased-lb} can be made \emph{natural} in the sense of Razborov-Rudich. More specifically, it would actually be sufficient to develop an efficient algorithm with the following behavior: for some absolute constant $c \in\mathbb{N}$, the algorithm takes as input a uniformly random set $A \subseteq \mathbb{F}_2^n$, $|A|= n^c$, and with nonnegligble probability must output a certificate that $A$ is a $\frac{1}{4}$-biased probability space. See Section~\ref{sec:avoid-and-linear-attack} for more details.

\subsection{Prior Work on Data Structure Lower Bounds}\label{subsec:prior}
...
We introduce some basic notation and definitions which will be used throughout. We strongly encourage the reader not to skip Section~\ref{subsec:gen-vs-cell} which defines our notation for locally computable generators and cell probe data structures, and establishes the connection between them which will be used in the rest of the paper.
\subsection{Basics}\label{subsec:basic-prelims}
For a natural number $N$ use $\N =\{1,\ldots,N\}$. For a finite set $V$ and integer $k$ we use $\binom{V}{k}$ to denote the set of subsets of $V$ of size $k$. If $X$ is a finite set, $\phi:X \to \mathbb{R}$ we write $\E_{x\in X} \phi(x)$ as shorthand for $|X|^{-1}\sum_{x \in X} \phi(x)$.
We will use boldface variables e.g. $\bm{v}$ exclusively for random variables. When $\bm{v}$ is a random variable and $A$ is a set, we write $\bm{v} \in A$ to indicate that the support of $\bm{v}$ is contained in $A$. When an expression references only a single random variable $\bm{v}$ we will use $\E \phi(\bm{v})$ for some $\phi: \supp(\bm{v}) \to \mathbb{R}$ to mean that the expectation is taken with respect to $\bm{v}$; this is in contrast to the notation $\E_{x\sim X} \phi(x)$ above, where we always include the subscript.
...
In Section \ref{subsec:k-wise-lbs} we will deal with various vector and matrix norms, whose (standard) definitions we review here:
...
\[
\E \prod_{x\in X} \bm{f}_x \in [-\gamma,\gamma]
\]
...
\begin{lemma}[\cite{naornaor}]\label{lem:bias-vs-indep}
...
\end{theorem*}
We elaborate on our notion of explicit: for the purposes of our range avoidance algorithms in Section~\ref{sec:avoid-and-linear-attack}, we can take explicit to mean, there is a uniform algorithm which, given $k,N,\epsilon$ will run in time $\poly(N, k, \frac{1}{\epsilon})$ and output a list of strings in $\{\pm 1\}^N$, such that the uniform distribution on these strings is $(\gamma,k)$-biased. The distributions in \cite{naornaor} are also explicit in the more standard/informal sense, being supplied by a direct construction. 

\subsection{Generators and Data Structures}\label{subsec:gen-vs-cell}
We now define the two main objects of study in our work: the cell probe model of static data structures on the one hand, and locally-computable generators on the other. We then state the tight connection between them, giving some more detail than was supplied in the introduction. Broader implications of this connection will be explored in more depth in Section~\ref{sec:connections}.

\begin{definition}[Cell Probe Model]
Let $F: \N \times [M] \to \{0,1\}$, which we refer to as a ``data structure problem.'' Elements $x \in \N$ are referred to as ``queries,'' $y \in [M]$ as ``datapoints.'' We say that $F$ has cell probe data structures with space complexity $S$, time complexity $t$, and word length $w$ if there exist procedures of the following form:
\begin{enumerate}
...
\begin{observation}\label{obs:main-correspondence}
...
The above discussion leaves out some complications of the argument; for a given $G,f$ we will actually construct a small collection of $t$-XOR CSPs, and can only guarantee that \emph{one of them} has a large value in the case $f \in \rng(G)$. To obtain good parameters in our main reduction (Theorem~\ref{thm:cell-csp}) we also need some additional arguments more specially tailored to the cell probe model which do not seem to have a direct analogue in the CSP-refutation literature. 
...
we define
\[
\mathrm{Val}(\mathcal{C}, f,R):= \bigl|\E_{x \in X} f_x  \prod_{i \in c_x}R(i)\bigr|
\]
...
\begin{observation}\label{obs:multilin}
For any $\mathcal{C},f$,
\[
\max_{R\in \{-1,1\}^V} \mathrm{Val} (\mathcal{C},f,R) = \max_{R\in [-1,1]^V} \mathrm{Val} (\mathcal{C},f,R) \geq \max_{R\in \{0,1\}^V} \mathrm{Val} (\mathcal{C},f,R) 
\]
\end{observation}
...
\end{proof}
The right-hand side in Observation~\ref{obs:multilin} may be seen as a kind of hypergraph discrepancy parameter. Viewing $(c_x)_{x \in X}$ as a $t$-uniform (multi-)hypergraph over vertex set $V$ with hyperedges indexed by $X$, we may think of $f \in \{ \pm 1\}^X$ as assigning a sign to each hyperedge. Then $ \max_{R\in \{0,1\}^V} \mathrm{Val} (\mathcal{C},f,R) $ is the maximum, over all subsets of vertices $U \subseteq V$, of the sum of the values $f_x$ with $x$ ranging over hyperedges in the sub-hypergraph induced by $U$. The smallness of $ \max_{R\in \{0,1\}^V} \mathrm{Val} (\mathcal{C},f,R) $ indicates that every large induced subhypergraph has its edges colored by $f$ in an approximately balanced way.
...
\begin{theorem}\label{thm:cell-csp}
...
\end{align*}
Using Observation~\ref{obs:multilin}, we may replace the boolean assignment $R_f\in \B^V$ by a $\{\pm 1\}$-assignment $\tilde{R}_f \in \{\pm 1\}^V$ achieving a value at least as large, so overall we conclude that $\Opt(\mathcal{C}^\p{\pi^f,b^f},f) \geq 2^{-tw-1}$.
...
In the terminology of Definition~\ref{def:csp-pseudo}, we aim to prove that limited independence implies CSP-pseudorandomness (within a certain regime of parameters). 

We accomplish this in the following way: starting with any $t$-XOR scheme $\mathcal{C}$ indexed by $\N$, we construct a certain ensemble of matrices indexed by $\N$, which has the property that whenever we take a \emph{signed sum} of these matrices using $f$ as a signing, we have that the $\infty\to1$ norm of the resulting matrix is lower bounded by $\Opt(\mathcal{C},f)$. We then prove that, when $\bm{f}$ is chosen from a pseudorandom distribution with suitable limited-independence properties, the operator norm of a random matrix sum signed by $\bm{f}$ will be small with high probability. The proof of this latter claim is delayed to the next subsection (Section~\ref{subsec:infty-to-one-tail}).
...
\begin{lemma}\label{lem:kikuchi-warmup}
...
\end{enumerate}
\end{theorem}
The construction is a straightforward generalization of that in Lemma~\ref{lem:kikuchi-warmup}: identifying $[m]$ with $\binom{V}{L}$, we set $A_x(a,b) = \mathbbm{1}\{ c_x = a \Delta b\}$ where $\Delta$ denotes symmetric difference. The proof is also essentially identical to that of the special case Lemma~\ref{lem:kikuchi-warmup} and so we relegate it to the appendix (Section~\ref{app:kikuchi}). From now on we will refer only to the more general Theorem~\ref{thm:kikuchi} and leave behind the special case Lemma~\ref{lem:kikuchi-warmup}.

It remains to show that if $\mathcal{A}$ is a permutation ensemble with sufficiently small dimension and large average degree, then for any random vector $\bm{f}$ exhibiting suitable \emph{limited independence} properties, we will have $\norm{\mathcal{A}[\bm{f}]}_{\infty \to 1}$ small with high probability. We defer the proof to the next subsection (Section~\ref{subsec:infty-to-one-tail}), but state the bounds here:
\begin{theorem}\label{thm:infty-one-tail-bound}
...
\]
\end{theorem}
Using Theorem~\ref{thm:kikuchi} we may directly translate this to a statement about XOR schemes: 
\begin{theorem}\label{thm:k-wise-csp}
...
Finally applying the above Theorem together with Theorem~\ref{thm:cell-csp}, we obtain our main cell probe lower bound:



\begin{theorem*} [Theorem \ref{thm:k-wise-lb} restated]
%\begin{theorem}
%\label{thm:k-wise-lb}
Let 
$F: [N] \times [M] \to \{\pm 1\}$ 
...
\begin{proof}
Set $\delta = 2^{-tw-1}$.
Say that $F$ has a time $t$, space $S$ data structure with $t$ even. Fix some $L \in \mathbb{N}$ satisfying $t \leq L \leq \frac{tS}{8}$ and $k \leq L \log N$ to be specified later, and assume that $\gamma \leq (tS)^{-L} 2^{-2k-1} \delta^{-k}$. Applying Theorem~\ref{thm:cell-csp} we may find $X \subseteq \N$ of size $|X| \geq \delta N$ and an XOR scheme $\mathcal{C}= (X,V,c)$, $|V|=K:=tS$ indexed by $X$ so that, with probability at least $\delta$ over $\bm{f}$, $\Opt(\mathcal{C},\bm{f}) \geq \delta$. Applying Theorem~\ref{thm:k-wise-csp} and using the fact that $2^{-k} < \delta$ (since $k > tw + 1)$, this is only possible provided
\[
...
\end{proof}
\subsection{Proof of Theorem~\ref{thm:infty-one-tail-bound}}\label{subsec:infty-to-one-tail}
We now prove Theorem~\ref{thm:infty-one-tail-bound} which is reproduced below for convenience:
\begin{theorem*}[Theorem~\ref{thm:infty-one-tail-bound}, restated]
Let $\mathcal{A}$ be a permutation ensemble in $\mathbb{R}^{m \times m}$ indexed by $\N$, let $\bm{f} \in \{\pm 1\}^N$ be a random vector which is $\gamma$-almost $k$-wise independent for $k$ even, and let $\delta > 0$ be sufficiently small. Assume the following relations are satisfied:
\begin{enumerate}
	\item $d(\mathcal{A})\geq   C\frac{1}{\delta^2} \log(\frac{1}{\delta}) m^{2/k}(k+\log m )$ for a sufficiently large universal constant $C$
	\item $\gamma \leq 2^{-2k-1}\delta^km^{-1}$ 
...
\begin{lemma}[Trace/Norm Inequality]\label{lem:trace-norm}
...
\begin{lemma}\label{lem:infty-to-spectral}
...
\begin{lemma}\label{lem:trace-poly}
...
We are now ready to prove Theorem~\ref{thm:infty-one-tail-bound}:
\begin{proof}[Proof of Theorem~\ref{thm:infty-one-tail-bound}]
...
We may apply the lower bound in Theorem~\ref{thm:k-wise-lb} to any problem $F: [N] \times [M] \to \B$ exhibiting suitable limited independence properties. If our goal is simply to minimize $M$ as a function of $N$, subject to proving the best quantitative bound and subject to $F$ being \emph{explicit}, our best course of action is to use any explicit construction of $\gamma$-almost $k$-wise independent distributions with minimum possible support size (i.e. minimum seed length), for example \cite{naornaor} or any of its subsequent improvements. However, it is also of interest to prove lower bounds for \emph{natural} problems which have some more concrete interpretation as an information retrieval task. 
We highlight here two such natural problems to which we can apply the lower bound in Theorem~\ref{thm:k-wise-lb}. Both pertain to evaluating low degree polynomials of some kind over a finite field.
...
We then have, as an immediate consequence of Theorem~\ref{thm:k-wise-lb} and the well known $d$-wise independent properties of degree $d$ polynomials:
\begin{theorem}\label{thm:polynom-lb}
...
\begin{lemma}[\cite{razborov-ramsey,savicky1995improved}]\label{lem:savicky}
...
\begin{gather}
\phi(x) = \mathop{\bigoplus}\limits_{i\leq m}\mathop{\bigwedge}\limits_{j \leq \log k + 1}  \mathop{\bigoplus}\limits_{u \leq 2n+2} \ell_{i,j,u} \label{eq:savicky}
\end{gather}
...
Combining Lemma~\ref{lem:savicky} and Theorem~\ref{thm:k-wise-lb} we arrive at the following cell probe lower bound for $\mathbb{F}_2\dash\mathsf{Eval}_n^d$:
\begin{theorem}
...
Using Theorem~\ref{thm:fourier}, we obtain from any $\mathsf{NC}^0_t$ generator a sequence of $r$-XOR schemes with various orders $r \leq t$. We will deal with the schemes $(\mathcal{C}_u)_{ |u|<t}$ of order $<t$ using limited independence as in Section~\ref{subsec:k-wise-lbs}; for these we can hope to obtain upper bounds on $\mathrm{Val}(\mathcal{C}_u,\bm{f})$ which approach 0 at a reasonably fast rate, provided $S << N^{\frac{2}{(t-1)}}$. For the highest order scheme $\mathcal{C}_{[t]}$, we will only be able to obtain an upper bound on $\mathrm{Val}(\mathcal{C}_{[t]},\bm{f})$ that is somewhat bounded away from one; however, if these two upper bounds match up correctly, we still have room to complete the lower bound according to Theorem~\ref{thm:fourier}.
...
\begin{theorem}[\cite{smoothed}]\label{thm:moore}
...
We now use Theorem~\ref{thm:moore} to give a nontrivial upper bound on $\Opt(\mathcal{C},\bm{f})$ when $\bm{f}$ has small bias:
\begin{theorem}\label{thm:biased-val}
...
By Theorem~\ref{thm:moore}, there exists a collection of disjoint nonempty sets $E_1,\ldots,E_r \subseteq \N$, $r \geq \epsilon \cdot L^{-1}(\frac{K}{L})^{\frac{t}{2}} \log^{4t}K$, such that for each $j \leq r$ we have $\mathop{\Delta}\limits_{x \in E_j} c_x = \emptyset$ where $\Delta$ denotes the symmetric difference operator.
We now observe, as in \cite{FKO,smoothed}, the following: for any $f$, if $\prod_{x \in E_j} f_x = -1$, then \emph{for every} $R \in \{\pm 1 \}^V$, there exists $x \in E_x$ so that $f_x \neq \prod_{i \in c_x} R(i)$. This follows since
\begin{gather*}
...
\[
\E\prod_{j \in u} \bm{g}_j  = \E\prod_{j \in u}\prod_{x \in E_j} \bm{f}_x= \E\prod_{x \in \cup_{j\in u} E_j} \bm{f}_x \in [-\gamma,  \gamma]
\]
...
% \begin{gather*}
% (\frac{r}{3})^{c}\Pr[|\sum_j \bm{g}_j| > \frac{r}{3}] \leq  \E (\sum_j \bm{g}_j)^c =  \sum_{j_1,\ldots,j_c}\E \prod_{w \leq c} \bm{g}_{j_w} 
% \leq \gamma r^c  + (c/2)!r^{c/2}, \text{ thus}\\
% \Pr[|\sum_j \bm{g}_j| > \frac{r}{3}] \leq 3^{c}\gamma + (c/2)! r^{-c/2} 3^c 
% \end{gather*}
\begin{gather*}
\Pr[|\sum_j \bm{g}_j| > \frac{8r}{10}] \leq  (\frac{8r}{10})^{-2}\E (\sum_j \bm{g}_j)^2 =  (\frac{8r}{10})^{-2}\sum_{j,j'}\E \bm{g}_j \bm{g}_{j'} 
...
\begin{observation}\label{obs:csp-padding}
Let $\mathcal{C} = (\N,V,c)$ be a $t$-XOR scheme. Then for any $t' \geq t$ there is a $t'$-XOR scheme $\mathcal{C'}=(\N,V',c')$ with $|V'| \leq |V|+ (t'-t)$ so that $\Opt(\mathcal{C},f) \leq \Opt(\mathcal{C'},f)$ for every $f \in \{\pm 1\}^X$.
\end{observation}
\begin{proof}
...
\begin{theorem*} [Theorem \ref{thm:biased-lb}, restated]
%\label{thm:biased-lb}
...
\end{theorem*}
Observe that for every $t>2$, $\frac{t-2}{2(t+2)} > 0$, hence we obtain lower bounds of the form $S \geq N^{\frac{t}{2} + \Omega(1)}$ for every odd $t$. Recall also Lemma~\ref{lem:bias-vs-indep} from Section~\ref{subsec:basic-prelims} in the preliminaries, which says that if a distribution is $\gamma$-biased then it is also $\gamma$-almost $k$-wise independent for any $k$. So we may strengthen our assumption on $F$ to the simpler statement that its columns support an $N^{-O(1)}$-biased distribution. Our proof here will follow very closely the arguments in Section 9 of \cite{smoothed}.
\begin{proof}
Assume $F$ has a data structure of space $S$ and time $t=O(1)$, and let $\bm{f}$ be the random vector supported on the columns of $F$ which is $\frac{1}{4}$-biased and $N^{-c}$-almost $\log N$-wise independence for $c$ a suitably large constant. By Theorem~\ref{thm:fourier} we may find $X\subseteq \N$, $|X| \geq 2^{-2^t}N$, and XOR schemes $(\mathcal{C}_u)_{ u \subseteq [t]}$ all indexed by $X$, each having $O(S)$ variables, so that $\mathcal{C}_u$ is a $|u|$-XOR scheme, and 
$   \max\limits_{u \subseteq [t], |u| < t} \mathrm{Val}(\mathcal{C}_u,f) \geq (1- \mathrm{Val}(\mathcal{C}_{[t]},f))2^{-t}$ for every column $f$ of $F$. Using Observation~\ref{obs:csp-padding} we may assume, up to increasing the number of variables of each XOR scheme by at most $t$, that $\mathcal{C}_u$ is a $(t-1)$-XOR scheme for every $|u|<t$. 
Set $L = S^{\frac{1}{t+2}}$, and assume that $N = \tilde{O}(1) \cdot \Bigl(\frac{S}{L}\Bigr)^{\frac{t}{2}}L$ where $\tilde{O}(1)$ is some sufficiently large $\poly\log N$ term. Applying Theorem~\ref{thm:biased-val}, with probability at least $\frac{1}{5}$, we have
\[
1-\Opt(\mathcal{C}_{[t]},\bm{f}) \geq \tilde{O}(1)\cdot  \frac{S^{t/2}}{ \Bigl(\frac{S}{L}\Bigr)^{\frac{t}{2}}L} = \tilde{O}(1) \cdot L^{-1} 
\]
On the other hand, using the fact that $\bm{f}$ is $\gamma$-almost $\log N$-wise independent and supported on the columns of $F$, with $\gamma \leq 2^{-O(tk)} = N^{-O(1)}$, we conclude using Theorem~\ref{thm:k-wise-csp} (and the fact that $t-1$ is even) that for every $|u|<t$ we have:
\[
\Pr[\Opt(\mathcal{C}_u,\bm{f}) \geq \delta] \leq N^{-1} << 2^{O(t)}, \quad \text{ provided }N  \geq \omega( \frac{1}{\delta^2}\log \frac{1}{\delta} S^{\frac{t-1}{2}} \log N)
\]
...
The only reason we needed to assume $t$ is odd is in our application of Theorem~\ref{thm:k-wise-csp}; if this theorem could be generalized to odd-arity XOR schemes (which seems attainable using methods for odd-arity CSPs, e.g. \cite{smoothed}), the results here would be valid for all $t > 2$.
...
% \begin{theorem}\label{thm:explicit-k-wise}
...
% \]
% \end{theorem}
% \begin{theorem}\label{thm:explicit-biased}
...
\section{Algorithmic Implications of Our Lower Bounds}\label{sec:avoid-and-linear-attack}
\subsection{Range Avoidance}
As referenced in the introduction, our main correspondence (Observation~\ref{obs:main-correspondence}) immediately implies that explicit cell probe lower bounds yield $\mathsf{NP}$ oracle algorithms for range avoidance. We give a more formal version of this statement here:
\begin{lemma}\label{lem:lb-yields-pnp-avoid}
...
\end{lemma}
\begin{proof}
By Observation~\ref{obs:main-correspondence}, the data structure lower bound for $F$ implies that one of its columns must lie outside the range of $G$. We may use a $\mathsf{NP}$ oracle to test each column in order and find the lexicographically first which lies outside the range of $G$.
\end{proof}
...
For one of our lower bounds (Theorem~\ref{thm:k-wise-lb}), we will be able to analyze the argument in a non black box way and remove the $\mathsf{NP}$ oracle. For Theorem~\ref{thm:biased-lb} we are unable to make such an improvement. We start with this latter result, where using known constructions of small bias probability spaces (\cite{naornaor}) we get:

\begin{theorem*}[Theorem \ref{thm:pnp-avoid-odd}, restated]
Let $t = O(1)$ be a fixed odd number. There is a polynomial time algorithm which outputs a list of strings in $\{0,1\}^m$, such that for any $\mathsf{NC}^0_t$ generator $G: \B^n \to \B^m$ with
\[
m \geq  \tilde{O}( n^{\frac{t}{2} - \frac{t-2}{2(t+2)}})
\]
...
We now move on to our first lower bound from Theorem~\ref{thm:k-wise-lb}, which applies to more general adaptive generators with larger word lengths. Compared to Theorem~\ref{thm:pnp-avoid-odd}, we will be able to remove the use of an $\mathsf{NP}$ oracle, and we will also be able to solve even the \emph{remote point} problem for $G$, finding a string which is $(\frac{1}{2} -\epsilon)$-far in relative hamming distance from every string in the range of $G$ for an arbitrarily small constant $\epsilon$.
\begin{theorem*}[Theorem \ref{thm:polytime-avoid}, restated]
...
\end{theorem}
\begin{proof}[Proof of Theorem~\ref{thm:cell-csp-correlation}]
We follow the proof of Theorem~\ref{thm:cell-csp} and indicate only those parts which need to change. The definitions of $\mathcal{C}^\p{\pi,b}$, $X^\p{\pi,b}$ remain as before; in this case we discard any such pair with $|X^\p{\pi,b}| < \epsilon2^{-tw-1}N$. Let $f,g$ be as in the statement of the theorem, and let $E_{g}$ be a canonical preimage of $g$ under $G$. For each $\pi$ let $
...
\[
\epsilon \leq \mathrm{Cor}(f,g)  = |\sum_{\pi\in \Sigma^t} \frac{|\tilde{X}^\p{\pi,g}|}{N} \E\limits_{x \sim \tilde{X}^\p{\pi,g}} f_xg_x|  \leq \sum_{\pi\in \Sigma^t} \frac{|\tilde{X}^\p{\pi,g}|}{N} \mathrm{Cor}_{\tilde{X}^\p{\pi,g}}(f,g)
\]
...
% \todo{something}, and $|\E\limits_{x \sim \tilde{X}^\p{\pi^{f,f'},f'}} f_xf'_x| \geq $ \todo{something}. 
...
\end{proof}
Using this in conjunction with the main ideas in Section~\ref{subsec:k-wise-lbs} we can now prove Theorem~\ref{thm:polytime-avoid}.
\begin{proof}[Proof of Theorem~\ref{thm:polytime-avoid}]
Given the generator $G$, we may apply Theorem~\ref{thm:cell-csp-correlation} to compute a list of $2^{tw+1}$ XOR schemes, so that whenever a string in $\{0,1\}^m \leftrightarrow\{\pm 1\}^m$ makes the value of all XOR schemes $< \epsilon2^{-tw-1}$, it must be $(\frac{1}{2}-\epsilon)$-far in relative hamming distance from $\rng(G)$. We will use \cite{naornaor} to construct a polynomial size list of candidate strings, such that one of them must make all such values small simultaneously which will guarantee that it is far form $\rng(G)$. To make our algorithm run in polynomial time, we need to certify that one of these strings from the list accomplishes this goal.
To perform this certification, we transform the XOR schemes into matrix ensembles using Theorem~\ref{thm:kikuchi} followed by Lemma~\ref{lem:infty-to-spectral}, and then compute the associated spectral norms. We can observe easily that each of the transformations prescribed in Theorems~\ref{thm:cell-csp} and \ref{thm:kikuchi} and in Lemma~\ref{lem:infty-to-spectral} can be performed efficiently.
Using \cite{naornaor} as indicated above, we can construct in time $2^{O(ktw)}$ a list of $\leq m^{O(tw)}$ strings which support a $2^{-O(ktw)}$-almost $k$-wise distribution for $k= t\log N$. By Theorem~\ref{thm:k-wise-csp}, one of the strings in this list will pass all of the $2^{tw+1}$ spectral tests define above; our algorithm outputs the first such string.
\end{proof}
...
\begin{corollary*}[Corollary \ref{cor:mossel-improvement}, restated]
...
\section{Towards Stronger Lower Bounds}\label{sec:connections}


In this section we discuss some of the broader implications of Observation~\ref{obs:main-correspondence} connecting locally-computable generators to cell probe lower bounds. Our focus here will be on the setting of nonadaptive bit probe data structures with time complexity $t = O(1)$, which correspond via Observation~\ref{obs:main-correspondence} to the class of $\mathsf{NC}^0$ generators. More specifically, we are interested in the problem of obtaining lower bounds of two kinds, which we refer to as \emph{strong} and \emph{super-strong} respectively. In each setting, our goal is to construct lower bounds for a problem $F$ with $N$ queries; $N$ will be the main complexity parameter. We seek lower bounds on the space as a function of $N$ and the time $t$; subject to obtaining such a lower bound and the problem being explicit, we'd like to minimize the number of datapoints $M$ of our problem as a function of $N$. We will parameterize each goal by a growth rate $M(N)$, where the goal gets more difficult the slower $M$ grows:

\begin{goal}[Strong Lower Bounds with $M(N)$ Datapoints]\label{goal:strong}
...
\end{goal}
\begin{goal}[Super-Strong Bit Lower Bounds with $M(N)$ Datapoints]\label{goal:super-strong}
...
\end{goal}
In the most ambitious case we aim for $M(N)= \poly(N)$, and in the most general case we aim for $M(N) = \exp(N^{o(1)})$. We observe that nonconstructive counting arguments imply that Goal~\ref{goal:strong} is achievable by a nonexplicit problem once $M \geq O(t\log N)$, while Goal~\ref{goal:super-strong} is achievable by a nonexplicit problem once $M \geq O(tN\log N)$, so even in the ambitious regime $M = \poly(N)$ neither goal is vacuous in an information-theoretic sense.
Note that our main lower bounds in Sections \ref{sec:indep-lbs} and \ref{sec:nonadaptive-lbs} are of the form $S \geq N^{\delta_t}$ where $\lim\limits_{t \to \infty} \delta_t = 0$ and hence they do not achieve either of these goals. In this section we discuss some barriers and approaches to achieving lower bounds of these kinds, and connect these problems to other topics in complexity theory. More specifically we will discuss barriers for both problems, while our ``approaches'' will only apply to the first (Goal \ref{goal:strong}); we will see shortly that super-strong lower bounds (Goal \ref{goal:super-strong}) are likely to be completely out of reach of current mathematical methods.
...
In this more formal terminology, our statement of Goal~\ref{goal:strong} should be interpreted as follows: define an explicit \emph{family} of problems $F_N:[N] \times [M(N)] \to \B$ for each $N$, so that $M(N) \leq \exp(N^{o(1)})$, and $F_N$ requires space $\Omega(N^\epsilon)$ for time $t$ data structures, where $\epsilon,t$ are as in the statement of Goal~\ref{goal:strong}, and $N$ goes to infinity after these are fixed.
...
\subsection{Consequences of Explicit Bit Probe Lower Bounds}\label{subsec:explicit-vs-avoid}
Recall Lemma~\ref{lem:lb-yields-pnp-avoid} from the previous section, which was a direct corollary of our main correspondence (Observation~\ref{obs:main-correspondence}): every explicit data structure lower bound yields a $\mathsf{NP}$ oracle range avoidance algorithm for locally computable generators with corresponding parameters.
We apply this as follows:
\begin{theorem*}[Theorem \ref{thm:explicit-vs-avoid}, restated]
~
\begin{enumerate}
    \item If explicit super-strong bit probe lower bounds with subexponentially many datapoints (Goal~\ref{goal:super-strong}, $M = \exp(N^{o(1)})$) are achievable, then $\mathsf{EXP^{NP}} \not\subseteq \mathsf{NC}^1$.
    \item If explicit strong bit probe lower bounds with polynomially many datapoints (Goal~\ref{goal:strong}, $M=\poly(N)$) are achievable, then there is a polynomial time $\mathsf{NP}$ oracle algorithm for $\mathsf{NC}^0$ range avoidance with polynomial stretch.
\end{enumerate}
\end{theorem*}
\begin{proof}
(1) If Goal~\ref{goal:super-strong} with $M(N) = \exp(N^{o(1)})$ is achievable than we can solve $\mathsf{NC}^0_t$ range avoidance in the stretch regime $n \mapsto n + n^{o(1)}$ in subexponential time with an $\mathsf{NP}$ oracle using Lemma~\ref{lem:lb-yields-pnp-avoid}. By \cite{RSW}, this implies the separation $\mathsf{EXP^{NP}} \not\subseteq \mathsf{NC}^1$.

(2) By well known composition reductions (see \cite{Korten}), we may reduce any $\mathsf{NC}^0$ range avoidance $G: \B^n \to \B^{n^{1+ \Omega(1)}}$ to a second $\mathsf{NC}^0$ instance $G': \B^n \to \B^{n^c}$ where $c=O(1)$ is an arbitrarily large constant in polynomial time with an $\mathsf{NP}$ oracle. If Goal~\ref{goal:strong} is achievable than by Lemma~\ref{lem:lb-yields-pnp-avoid} we can solve range avoidance in this regime using an $\mathsf{NP}$ oracle in polynomial time.
\end{proof}

In (2) above, if we instead achieved Goal~\ref{goal:strong} with $M = \exp(\poly \log N)$ we would obtain a quasipolynomial time algorithm; if we achieved it with $M=\exp(N^{o(1)})$ we would obtain a subexponential time algorithm. Obtaining $\mathsf{P^{NP}}$ algorithms for $\mathsf{NC}^0$ range avoidance in the polynomial stretch regime was explicitly posed as an open problem in \cite{RSW}.

\subsection{Natural Bit Probe Lower Bounds Break $\mathsf{NC^0}$ PRGs}\label{subsec:natural-vs-prg}
...
\begin{definition}[Natural Bit Probe Lower Bounds]\label{def:natural-props}
...
\begin{lemma}\label{lem:hybrid-natural-prop}
...
\end{lemma}
Combining these two lemmas with our Lemma~\ref{lem:hybrid-natural-prop} we arrive at:

\begin{theorem*}[Theorem \ref{thm:nat-prop-barrier}, restated: Natural Proofs Barriers for Bit Probe Lower Bounds]
~\begin{enumerate}
...
\end{definition}
Just as the distinction between adaptive and nonadaptive data structures is irrelevant with respect to lower bounds achieving Goal~\ref{goal:strong}, so too is the distinction between general and homogeneous data structures:
\begin{lemma}
...
Let $F: X \times Y \to \{0,1\}$ be a two-party communication problem. We say that $F$ has a $(t,k)$-parallel protocol if it can be computed in the following way:
\begin{enumerate}
...
\begin{observation}\label{obs:parallel-channel}
...
\end{observation}
We refer to this as equivalence, since w.r.t. Goal~\ref{goal:strong} the difference between a space lower bound $S$ and $tS$ is irrelevant. 
\begin{proof}
...
\end{proof}
We can then restate Goal~\ref{goal:strong} as follows:
\begin{goal}[Strong Parallel Communication Lower Bounds for a problem with $M$ Columns]\label{goal:equiv}
...
\end{goal}
In this setting we will set $M = 2^m$ for some $m$, and identify $[M]$ with $\B^m$. In this case, achieving $m \leq O(n)$ for Goal~\ref{goal:equiv} is equivalent to our most ambitious goal of achieving $M \leq \poly(N)$ for Goal~\ref{goal:strong}; we would also be happy with $m \leq 2^{o(n)}$ in Goal~\ref{goal:equiv}, which corresponds to our most general acceptable form of Goal~\ref{goal:strong} with $M \leq \exp(N^{o(1)})$.

While our primary intention in investigating the parallel channel model is as an avenue through which to derive new cell probe lower bounds, using Observation~\ref{obs:parallel-channel} we can use our lower bounds from Sections \ref{sec:indep-lbs} and \ref{sec:nonadaptive-lbs} to derive new lower bounds in this communication model, for example:
\begin{lemma}
...
\begin{conjecture}[Lifting $C^2_k$ To Parallel Channel Complexity]\label{conj:lifting}
...
\end{conjecture}
The above conjecture would imply a lower bound achieving Goals~\ref{goal:strong}/\ref{goal:equiv} for the specific explicit problem $\mathrm{MAJ}^n \circ \mathrm{Ind}_\ell^n$, $\ell \leq \poly(n)$. In this parameter setting, we would have a problem with $N = \ell^n = 2^{O(n\log n)}$ rows and $M = 2^{\poly(n)}$ columns, i.e. $M$ is at most quasipolynomial in $N$.
...
\end{lemma}
\begin{proof}
Say that $F(x,y) = h(g_1(x,y),\ldots,g_t(x,y))$ with $g_j$ having one way communication complexity at most $k$. So $g_j(x,y) = R_j(\Pi_j(x),y)$ for some $\Pi_j: \B^n \to \B^k$, $R_j: \B^k \times \B^n \to \B$. Hence
\[
F(x,y)=\bigvee_{\xi \in h^{-1}(1)} \bigwedge_{\substack{j \leq t,\\ z \in \B^k}} \mathbbm{1}\{\Pi_j(x) \neq z\} \lor \mathbbm{1}\{R(z,y) = \xi_j\}
\]
\end{proof}

This means that any explicit problem $F$ satisfying $\mathsf{\Sigma_2^{cc}}(F)\geq \Omega(n)$ would automatically achieve Goal~\ref{goal:strong}. This also yields, via Lemma~\ref{lem:hybrid-natural-prop}, a natural properties barrier for the measure $\mathsf{\Sigma_2^{cc}}(\cdot)$:
\begin{lemma}
...
These facts gives some indication that proving $\Omega(n)$ lower bounds on $\mathsf{\Sigma_2^{cc}}$ complexity may far overshoot Goal~\ref{goal:strong} in terms of difficulty. 
% Nonetheless the connection to cell probe lower bounds and the natural properties barrier is apparently new. 
We make an additional note that, using basically the same proof as above, it is also possible to strengthen Lemma~\ref{lem:sigma_2} to general adaptive data structures with arbitrary word length:
\begin{lemma}[Strengthening of Lemma~\ref{lem:sigma_2}]
...
\begin{lemma}\label{lem:replacement-rank}
...
\end{proof}
Hence, exhibiting an explicit matrix $F \in \B^{N \times M}$ satisfying $\rrank_p(F) \geq N^{\epsilon}$ for some fixed $\epsilon>0$ and all primes $p =O(1)$ would solve Goal~\ref{goal:strong}. Currently the best lower bound we know of for $\rrank_p(\cdot)$ is via the inequality $\rrank_p(A) \leq p\cdot \rank(A)^{(p-1)}$ which follows by expressing the predicate $\phi: \mathbb{F}_p \to \B$ as a degree $p-1$ polynomial over $\mathbb{F}_p$ using Fermat's little theorem; this never gives a bound better than $N^{\frac{1}{p-1}}$ which is clearly insufficient for our purposes. 
As in the case of $\mathsf{\Sigma_2^{cc}}$ complexity, combining Lemma~\ref{lem:replacement-rank} with Lemma~\ref{lem:hybrid-natural-prop} yields a natural properties barrier for lower bounding $\rrank_p(F)$.



\section*{Acknowledgment}
The authors would like to give a special thanks to Pravesh Kothari, who, after hearing about a preliminary version of this work, made various suggestions which dramatically simplified and improved our core lower bound results in Section~\ref{sec:indep-lbs}. The authors would also like to thank the Simons Institute for the Theory of Computation where some portions of this work were completed.
\bibliographystyle{alpha}
\bibliography{Cell.bib}


\appendix
% \section{Delayed Proofs }
% \todo{condense to one section if we don't delay anything else}
\section{Kikuchi Method}\label{app:kikuchi}
We prove here Theorem~\ref{thm:kikuchi} from Section~\ref{sec:indep-lbs}. This argument occurs in \cite{wein2019kikuchi,smoothed} and we only need to adapt things to our notation:
\begin{theorem}[Theorem \ref{thm:kikuchi}]
Let $\mathcal{C} = (\N,V,c)$ be a $t$-XOR scheme, $|V|=K$, and $t$ even. For any $t \leq L \leq \frac{K}{8}$, we may associate with $\mathcal{C}$ a permutation ensemble
$\mathcal{A}$ indexed by $\N$ with the following properties:
\begin{enumerate}
	\item $\mathcal{A}$ lives in $\mathbb{R}^{m \times m}$ for $m = \binom{K}{L}$
